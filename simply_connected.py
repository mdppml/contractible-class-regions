...
ONE_GRAY_LEVEL_L2 = math.sqrt(
    IMAGE_HEIGHT * IMAGE_WIDTH * sum((1.0 / (255.0 * value)) ** 2 for value in IMAGENET_STANDARD_DEVIATION)
)
...
def build_fill_config(args) -> FillConfig:
    ...
    return FillConfig(
        max_deepfool_iter=int(args.deepfool_iter),
        overshoot=float(args.overshoot),
        line_search_steps=int(args.ls_steps),
        base_grid_side=int(args.base_grid_side),
        min_quad_side_rel=float(args.min_quad_side_rel),
        stop_diameter_gray_rms=float(args.stop_diameter_gray_rms),
        sample_batch_size=int(args.sample_batch_size),
        label_batch_size=int(args.label_batch_size),
        grid_point_block=int(args.grid_point_block),
        repair_batch_size=int(args.repair_batch_size),
        checkpoint_every_level=bool(args.checkpoint_every_level),
        keep_last_checkpoints=int(args.keep_last_checkpoints),
        checkpoint_interval=int(args.checkpoint_interval),
        size_metric_sample_quads=int(args.size_metric_sample_quads),
        size_filter_batch_size=int(args.size_filter_batch_size),
        use_adaptive_grid_steps=bool(args.use_adaptive_grid_steps),
        min_grid_steps=int(args.min_grid_steps),
        max_grid_steps=int(args.max_grid_steps),
        max_runtime_s=max_runtime_s,
        use_boundary_param_cache=bool(args.use_boundary_param_cache),
        boundary_reference_grid=int(args.boundary_reference_grid),
    )
    ...
    while True:
        try:
            if torch.cuda.is_available() and torch.cuda.device_count() > 0:
                ...
                print(f"CUDA ready: device_count={torch.cuda.device_count()} device_name={torch.cuda.get_device_name(0)}", flush=True)
                return device
        ...
        if time.time() >= deadline:
            ...
            raise RuntimeError(
                f"CUDA is not available after waiting {cuda_wait_seconds} seconds. "
                f"CUDA_VISIBLE_DEVICES={visible_devices}. Last error: {last_error}"
            )
        print(
            f"CUDA not ready yet. CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')} "
            f"Last error: {last_error}. Sleeping {cuda_retry_sleep}s.",
            flush=True,
        )
...
def find_quad_image_path(image_dir: Path, quad_id: int, corner_name: str, filename_width: int) -> Path:
    candidates = [
        f"{quad_id}{corner_name}.jpeg",
        f"{quad_id}{corner_name}.jpg",
        f"{quad_id}{corner_name}.png",
    ]
    if filename_width > 0:
        candidates.extend(
            [
                f"{quad_id:0{filename_width}d}{corner_name}.jpeg",
                f"{quad_id:0{filename_width}d}{corner_name}.jpg",
                f"{quad_id:0{filename_width}d}{corner_name}.png",
            ]
        )
...
def quad_size(corner00: torch.Tensor, corner10: torch.Tensor, corner01: torch.Tensor, corner11: torch.Tensor):
    ...
    return {
        "min_edge_l2": float(min(edges)),
        "mean_edge_l2": float(mean_edge),
        "max_edge_l2": float(max_edge),
        "mean_diag_l2": float(sum(diagonals) / len(diagonals)),
        "max_diag_l2": float(max(diagonals)),
        "min_all_l2": float(min(all_distances)),
        "mean_all_l2": float(sum(all_distances) / len(all_distances)),
        "diameter_l2": float(diameter),
        "diameter_norm_rms": l2_to_norm_rms(diameter),
        "diameter_gray_rms": l2_to_gray_rms(diameter),
        "max_edge_norm_rms": l2_to_norm_rms(max_edge),
        "max_edge_gray_rms": l2_to_gray_rms(max_edge),
        "mean_edge_norm_rms": l2_to_norm_rms(mean_edge),
        "mean_edge_gray_rms": l2_to_gray_rms(mean_edge),
        "area_l2_sq": float(area),
    }
...
def quad_size_summary(vertices: VertexMap, quads: Sequence[Quad], max_quads: int):
    ...
    return {
        "frontier": int(len(quads)),
        "sample_n": int(len(rows)),
        "mode": "exact" if len(quads) <= max_quads else "sampled",
        "diameter_l2_min": diameter["min"],
        "diameter_l2_mean": diameter["mean"],
        "diameter_l2_max": diameter["max"],
        "diameter_gray_min": diameter_gray["min"],
        "diameter_gray_mean": diameter_gray["mean"],
        "diameter_gray_max": diameter_gray["max"],
        "max_edge_l2_min": max_edge["min"],
        "max_edge_l2_mean": max_edge["mean"],
        "max_edge_l2_max": max_edge["max"],
        "max_edge_gray_min": max_edge_gray["min"],
        "max_edge_gray_mean": max_edge_gray["mean"],
        "max_edge_gray_max": max_edge_gray["max"],
        "mean_edge_l2_min": mean_edge["min"],
        "mean_edge_l2_mean": mean_edge["mean"],
        "mean_edge_l2_max": mean_edge["max"],
        "mean_edge_gray_min": mean_edge_gray["min"],
        "mean_edge_gray_mean": mean_edge_gray["mean"],
        "mean_edge_gray_max": mean_edge_gray["max"],
        "area_l2_sq_min": area["min"],
        "area_l2_sq_mean": area["mean"],
        "area_l2_sq_max": area["max"],
        "area_l2_sq_total_est": area_total,
    }
...
def new_repair_stats():
    return {
        ...
        "repair_attempts": 0,
        "repair_s": 0.0,
        "repair_gpu_sum_s": 0.0,
        "repair_iters_sum": 0,
        "repair_iters_max": 0,
        "repair_hit_max_iter": 0,
        "repair_amp_used": 0,
        "repair_amp_steps_sum": 0,
        "repair_val_tol_steps": 0,
        "repair_zero_grad_fallbacks": 0,
        "repair_random_fallbacks": 0,
        "repair_iter_le_5": 0,
        "repair_iter_6_10": 0,
        "repair_iter_11_25": 0,
        "repair_iter_26_50": 0,
        "repair_iter_51_100": 0,
        "repair_iter_gt_100": 0,
    }
...
def targeted_deepfool_l2_batch(model, batch: torch.Tensor, target_label: int, max_iter: int = 50, overshoot: float = 0.02, step_min: float = 1e-3, eps: float = 1e-12, val_tol: float = 1e-6):
    ...
    stats = {
        "iters": iteration_counts.detach().cpu(),
        "hit_target": hit_target.detach().cpu(),
        "hit_max_iter": hit_max_iter.detach().cpu(),
        "last_label": last_label.detach().cpu(),
        "val_tol_steps": val_tol_steps.detach().cpu(),
        "zero_grad_fallbacks": zero_grad_fallbacks.detach().cpu(),
        "random_fallbacks": random_fallbacks.detach().cpu(),
    }
...
def repair_batch_to_label(model, batch: torch.Tensor, target_label: int, max_iter: int, overshoot: float, line_search_steps: int, amp_steps: int = 12):
    ...
    for index in range(count):
        add_single_repair(
            stats,
            {
                "deepfool_iters": int(deepfool_stats["iters"][index].item()),
                "deepfool_hit_target": bool(deepfool_stats["hit_target"][index].item()),
                "deepfool_hit_max_iter": bool(deepfool_stats["hit_max_iter"][index].item()),
                "deepfool_val_tol_steps": int(deepfool_stats["val_tol_steps"][index].item()),
                "deepfool_zero_grad_fallbacks": int(deepfool_stats["zero_grad_fallbacks"][index].item()),
                "deepfool_random_fallbacks": int(deepfool_stats["random_fallbacks"][index].item()),
                "used_amp": bool(used_amp[index].item()),
                "amp_steps_used": int(amp_count[index].item()),
            },
        )
    return output, ok.detach().cpu().tolist(), stats
...
def repair_vertex_keys(model, vertices: VertexMap, verified_vertices: set, keys: Sequence[ImageKey], target_label: int, config: FillConfig, stop_on_failure: bool = False):
    ...
    for start in range(0, len(keys), config.repair_batch_size):
        ...
        repaired_batch, ok_list, batch_stats = repair_batch_to_label(
            model,
            gpu_batch,
            target_label,
            max_iter=config.max_deepfool_iter,
            overshoot=config.overshoot,
            line_search_steps=config.line_search_steps,
        )
...
def subdivision_keys(quad: Quad):
    ...
    children = [
        (key00, key_bottom, key_left, key_center),
        (key_bottom, key10, key_center, key_right),
        (key_left, key_center, key01, key_top),
        (key_center, key_right, key_top, key11),
    ]
...
def bilinear_points(corner00: torch.Tensor, corner10: torch.Tensor, corner01: torch.Tensor, corner11: torch.Tensor, grid_u: torch.Tensor, grid_v: torch.Tensor) -> torch.Tensor:
    ...
    points = (
        corner00.unsqueeze(1) * weight00.view(1, -1, 1, 1, 1)
        + corner10.unsqueeze(1) * weight10.view(1, -1, 1, 1, 1)
        + corner01.unsqueeze(1) * weight01.view(1, -1, 1, 1, 1)
        + corner11.unsqueeze(1) * weight11.view(1, -1, 1, 1, 1)
    )
...
def coons_row(bottom, top, left_value, right_value, corner00, corner10, corner01, corner11, u_grid, v_value):
    ...
    corner_patch = (
        (1.0 - grid_u) * (1.0 - grid_v) * corner00.unsqueeze(0)
        + grid_u * (1.0 - grid_v) * corner10.unsqueeze(0)
        + (1.0 - grid_u) * grid_v * corner01.unsqueeze(0)
        + grid_u * grid_v * corner11.unsqueeze(0)
    )
...
def new_run_stats(target_label: Optional[int], config: FillConfig, coord_depth: int, coord_denom: int):
    return {
        "y": target_label,
        "initial_quad_ok": None,
        "levels_processed": 0,
        "quads_checked": 0,
        "quads_ok": 0,
        "quads_failed": 0,
        "quads_created": 0,
        "quads_size_accepted": 0,
        "repair_attempts": 0,
        "repair_failed": 0,
        "leaf_quads": 0,
        "resolution_leaf_quads": 0,
        "grid_points_checked": 0,
        "time_total_s": 0.0,
        "time_vertex_s": 0.0,
        "time_grid_s": 0.0,
        "time_subdivide_s": 0.0,
        "time_repair_s": 0.0,
        "time_repair_gpu_sum_s": 0.0,
        "time_grid_model_s": 0.0,
        "time_grid_cpu_s": 0.0,
        "time_grid_stack_s": 0.0,
        "time_size_filter_s": 0.0,
        "repair_iters_sum": 0,
        "repair_iters_max": 0,
        "repair_hit_max_iter": 0,
        "repair_amp_used": 0,
        ...
        "first_fail": None,
        "failed_depth": None,
        "failed_quad_side_rel": None,
        "min_quad_side_rel": float(config.min_quad_side_rel),
        "stop_diameter_gray_rms": float(config.stop_diameter_gray_rms),
        "effective_quad_side_rel": float(1.0 / coord_denom),
        "effective_quad_area_rel": float((1.0 / coord_denom) ** 2),
        "coord_depth": int(coord_depth),
        "coord_denom": int(coord_denom),
        "max_depth_observed": 0,
        "vertex_count": 4,
        "vertex_ok_count": 4,
        "frontier_remaining": 0,
        "root_diameter_l2": None,
        "root_max_edge_l2": None,
        "root_mean_edge_l2": None,
        "root_diameter_gray_rms": None,
        "original_straight_quad_area_l2_sq": None,
        "constructed_area_l2_sq": 0.0,
        "boundary_reference_area_l2_sq": None,
        "boundary_reference_grid": int(config.boundary_reference_grid),
        "boundary_reference_s": 0.0,
        "use_boundary_param_cache": bool(config.use_boundary_param_cache),
        "boundary_created_total": 0,
        "boundary_created_a_to_b": 0,
        "boundary_created_b_to_d": 0,
        "boundary_created_c_to_d": 0,
        "boundary_created_a_to_c": 0,
        "level_records": [],
    }
...
def print_area_table(stats) -> None:
    ...
    for record in records:
        ...
        print(
            f"{depth:5d} | "
            f"{int(record.get('frontier', 0)):8d} | "
            f"{int(record.get('size_pass', 0)):9d} | "
            f"{int(record.get('grid_pass', 0)):9d} | "
            f"{int(record.get('resolution_pass', 0)):8d} | "
            f"{int(record.get('fail', 0)):4d} | "
            f"{next_count:4d} | "
            f"{100.0 * accepted_area:15.6f} | "
            f"{100.0 * cumulative:17.6f} | "
            f"{100.0 * remaining_area:16.6f} | "
            f"{geom_here:15.6g} | "
            f"{geom_ratio:15.6g}"
        )
...
def fill_quad_surface(model, corner00: torch.Tensor, corner10: torch.Tensor, corner01: torch.Tensor, corner11: torch.Tensor, config: FillConfig, paths: RunPaths, checkpoint_prefix: str, target_label: Optional[int] = None):
    ...
    print(
        f"  root quad size: "
        f"diameter_l2={root_size['diameter_l2']:.6g} "
        f"diameter_norm_rms={root_size['diameter_norm_rms']:.6g} "
        f"diameter_gray_rms={root_size['diameter_gray_rms']:.6g} "
        f"max_edge_l2={root_size['max_edge_l2']:.6g} "
        f"max_edge_gray_rms={root_size['max_edge_gray_rms']:.6g} "
        f"mean_edge_l2={root_size['mean_edge_l2']:.6g} "
        f"mean_edge_gray_rms={root_size['mean_edge_gray_rms']:.6g} "
        f"original_straight_quad_area_l2_sq={root_size['area_l2_sq']:.6g} "
        f"one_gray_l2={ONE_GRAY_LEVEL_L2:.6g} "
        f"stop_diameter_gray_rms={config.stop_diameter_gray_rms:.6g} "
        f"stop_depth_diam_stop={stop_depth_diam} "
        f"stop_depth_edge_stop={stop_depth_edge} "
        f"stop_depth_diam_one_gray={stop_depth_one_gray} "
        f"stop_depth_diam_half_gray={stop_depth_half_gray} "
        f"boundary_param_cache={int(config.use_boundary_param_cache)} "
        f"boundary_reference_grid={int(config.boundary_reference_grid)}",
        flush=True,
    )
    ...
    for depth in range(coord_depth + 1):
        ...
        print(
            f"  level size: depth={depth} frontier={frontier_before} "
            f"size_mode={size_info['mode']} size_n={size_info['sample_n']} "
            f"size_s={size_s:.3f} "
            f"diam_l2_min={size_info['diameter_l2_min']:.6g} "
            f"diam_l2_mean={size_info['diameter_l2_mean']:.6g} "
            f"diam_l2_max={size_info['diameter_l2_max']:.6g} "
            f"diam_gray_min={size_info['diameter_gray_min']:.6g} "
            f"diam_gray_mean={size_info['diameter_gray_mean']:.6g} "
            f"diam_gray_max={size_info['diameter_gray_max']:.6g} "
            f"max_edge_l2_min={size_info['max_edge_l2_min']:.6g} "
            f"max_edge_l2_mean={size_info['max_edge_l2_mean']:.6g} "
            f"max_edge_l2_max={size_info['max_edge_l2_max']:.6g} "
            f"max_edge_gray_min={size_info['max_edge_gray_min']:.6g} "
            f"max_edge_gray_mean={size_info['max_edge_gray_mean']:.6g} "
            f"max_edge_gray_max={size_info['max_edge_gray_max']:.6g} "
            f"area_l2_sq_mean={size_info['area_l2_sq_mean']:.6g}",
            flush=True,
        )
        ...
        if depth >= coord_depth:
            ...
            print(f"  level timing: depth={depth} frontier={frontier_before} accepted_delta={len(accepted)-accepted_before} resolution_leaf={frontier_before} geom_area_l2_sq={geom_area:.6g} level_s={level_s:.3f} vertex_s={vertex_s:.3f} keys_s={keys_s:.3f} vertices={len(vertices)} verified={len(verified_vertices)}", flush=True)
        ...
        print(f"  size stop: depth={depth} checked={size_filter_info['checked']} accepted={size_filter_info['accepted']} remaining_for_grid={size_filter_info['remaining']} threshold_gray={config.stop_diameter_gray_rms:.6g} diam_gray_min={size_filter_info['min_gray']:.6g} diam_gray_mean={size_filter_info['mean_gray']:.6g} diam_gray_max={size_filter_info['max_gray']:.6g} accepted_area_l2_sq={size_filter_info['accepted_area_l2_sq']:.6g} size_filter_s={size_filter_s:.3f}", flush=True)
        if len(grid_frontier) == 0:
            ...
            level_s = time.perf_counter() - level_started
            print(f"  level timing: depth={depth} frontier={frontier_before} size_pass={len(size_accepted)} grid_frontier=0 geom_area_l2_sq={size_filter_info['accepted_area_l2_sq']:.6g} level_s={level_s:.3f} keys_s={keys_s:.3f} vertex_s={vertex_s:.3f} size_filter_s={size_filter_s:.3f} vertices={len(vertices)} verified={len(verified_vertices)} total_elapsed={time.perf_counter()-total_started:.1f}", flush=True)
            maybe_save_checkpoint(paths, checkpoint_prefix, depth, vertices, verified_vertices, accepted, frontier, stats, config)
            break
        ...
        print(f"  grid choice: depth={depth} grid_frontier={len(grid_frontier)} grid_steps={grid_steps} base_steps={base_steps} adaptive_steps={adaptive_steps} grid_capped={int(grid_capped)} grid_frontier_diam_gray_max={grid_size_info['diameter_gray_max']:.6g}", flush=True)
        ...
        if len(failing) == 0:
            ...
            level_s = time.perf_counter() - level_started
            print(f"  level timing: depth={depth} frontier={frontier_before} size_pass={len(size_accepted)} grid_pass={len(passing)} fail=0 grid_steps={grid_steps} samples={checked} grid_pass_area_l2_sq={grid_pass_area:.6g} level_s={level_s:.3f} vertex_s={vertex_s:.3f} grid_s={grid_s:.3f} size_filter_s={size_filter_s:.3f} grid_model_s={grid_info['model_s']:.3f} grid_cpu_s={grid_info['cpu_s']:.3f} grid_stack_s={grid_info['stack_s']:.3f} batches={grid_info['model_batches']} vertices={len(vertices)} verified={len(verified_vertices)}", flush=True)
            maybe_save_checkpoint(paths, checkpoint_prefix, depth, vertices, verified_vertices, accepted, frontier, stats, config)
            break
        ...
        print(
            f"  level timing: depth={depth} frontier={frontier_before} size_pass={len(size_accepted)} grid_pass={len(passing)} fail={len(failing)} next={len(frontier)} "
            f"grid_steps={grid_steps} samples={checked} size_area_l2_sq={size_filter_info['accepted_area_l2_sq']:.6g} grid_pass_area_l2_sq={grid_pass_area:.6g} "
            f"level_s={level_s:.3f} keys_s={keys_s:.3f} vertex_s={vertex_s:.3f} vertex_missing={vertex_stats['missing']} vertex_label_s={vertex_stats['label_s']:.3f} vertex_repair_s={vertex_stats['repair_s']:.3f} "
            f"size_filter_s={size_filter_s:.3f} grid_s={grid_s:.3f} grid_model_s={grid_info['model_s']:.3f} grid_cpu_s={grid_info['cpu_s']:.3f} grid_stack_s={grid_info['stack_s']:.3f} "
            f"grid_batches={grid_info['model_batches']} failed_early={grid_info['failed_early']} subdivide_s={subdivision_s:.3f} sub_label_s={subdivision_stats['label_s']:.3f} sub_repair_wall_s={subdivision_stats['repair_s']:.3f} "
            f"sub_repair_gpu_sum_s={subdivision_stats['repair_gpu_sum_s']:.3f} sub_repairs={subdivision_stats['repair_attempts']} repair_wall_avg_s={repair_wall_avg:.3f} repair_gpu_avg_s={repair_gpu_avg:.3f} "
            f"repair_avg_iter={repair_avg_iter:.2f} repair_max_iter={subdivision_stats['repair_iters_max']} repair_hit_max={subdivision_stats['repair_hit_max_iter']} amp_used={subdivision_stats['repair_amp_used']} amp_avg_steps={amp_avg_steps:.2f} "
            f"iter_hist=[<=5:{subdivision_stats['repair_iter_le_5']},6-10:{subdivision_stats['repair_iter_6_10']},11-25:{subdivision_stats['repair_iter_11_25']},26-50:{subdivision_stats['repair_iter_26_50']},51-100:{subdivision_stats['repair_iter_51_100']},>100:{subdivision_stats['repair_iter_gt_100']}] "
            f"boundary_new={subdivision_stats.get('boundary_created_total', 0)} boundary_edges=[ab:{subdivision_stats.get('boundary_created_a_to_b', 0)},bd:{subdivision_stats.get('boundary_created_b_to_d', 0)},cd:{subdivision_stats.get('boundary_created_c_to_d', 0)},ac:{subdivision_stats.get('boundary_created_a_to_c', 0)}] "
            f"sub_label_batches={subdivision_stats['label_batches']} sub_candidate_vertices={subdivision_stats['candidate_vertices']} sub_missing={subdivision_stats['missing']} new_vertices={len(vertices)-vertices_before} vertices={len(vertices)} verified={len(verified_vertices)} total_elapsed={time.perf_counter()-total_started:.1f}",
            flush=True,
        )
        cleanup_memory(device)
        maybe_save_checkpoint(paths, checkpoint_prefix, depth, vertices, verified_vertices, accepted, frontier, stats, config)
    ...
    if stats["reason"] == "success" and int(config.boundary_reference_grid) > 0:
        ...
        try:
            ...
            print(f"  boundary reference area: grid_steps={int(config.boundary_reference_grid)} area_l2_sq={reference_area:.6g} constructed_to_boundary_ref_ratio={ratio:.6g} boundary_ref_s={stats['boundary_reference_s']:.3f}", flush=True)
        except Exception as exc:
            stats["boundary_reference_area_l2_sq"] = None
            stats["boundary_reference_s"] = time.perf_counter() - reference_started
            print(f"  boundary reference area: FAILED reason={type(exc).__name__}: {exc} boundary_ref_s={stats['boundary_reference_s']:.3f}", flush=True)
    sync_model(model)
    stats["time_total_s"] = time.perf_counter() - total_started
    avg_iter = float(stats["repair_iters_sum"]) / max(1, int(stats["repair_attempts"]))
    print(
        f"  timing summary: total_s={stats['time_total_s']:.3f} vertex_s={stats['time_vertex_s']:.3f} grid_s={stats['time_grid_s']:.3f} grid_model_s={stats['time_grid_model_s']:.3f} "
        f"grid_cpu_s={stats['time_grid_cpu_s']:.3f} grid_stack_s={stats['time_grid_stack_s']:.3f} subdivide_s={stats['time_subdivide_s']:.3f} size_filter_s={stats['time_size_filter_s']:.3f} "
        f"repair_wall_s={stats['time_repair_s']:.3f} repair_gpu_sum_s={stats['time_repair_gpu_sum_s']:.3f} repair_attempts={stats['repair_attempts']} repair_avg_iter={avg_iter:.2f} "
        f"repair_max_iter={stats['repair_iters_max']} repair_hit_max={stats['repair_hit_max_iter']} repair_amp_used={stats['repair_amp_used']} size_accepted={stats['quads_size_accepted']} "
        f"boundary_param_cache={int(stats['use_boundary_param_cache'])} boundary_created_total={stats['boundary_created_total']} boundary_ref_s={float(stats.get('boundary_reference_s') or 0.0):.3f}",
        flush=True,
    )
...
def print_run_header(args, paths: RunPaths, quad_id: int, config: FillConfig) -> None:
    ...
    print(f"Max wall hours in Python: {float(args.max_wall_hours):.3f}", flush=True)
    print(f"Base grid side: {config.base_grid_side}", flush=True)
    print(f"Minimum quad relative side length: {config.min_quad_side_rel:.8g}", flush=True)
    ...
    print(f"Channels last: {bool(args.use_channels_last)}", flush=True)
    print(f"TF32: {bool(args.use_tf32)}", flush=True)
    print(f"Checkpoint every level: {config.checkpoint_every_level}", flush=True)
    print(f"Keep last checkpoints: {config.keep_last_checkpoints}", flush=True)
    print(f"Checkpoint interval: {config.checkpoint_interval}", flush=True)
    print(f"Size metric sample quads: {config.size_metric_sample_quads}", flush=True)
    print(f"Size filter batch size: {config.size_filter_batch_size}", flush=True)
    print(f"Adaptive grid steps: {config.use_adaptive_grid_steps}", flush=True)
    print(f"Min grid steps: {config.min_grid_steps}", flush=True)
    print(f"Max grid steps: {config.max_grid_steps}", flush=True)
    print(f"Boundary param cache: {config.use_boundary_param_cache}", flush=True)
    print(f"Boundary reference grid: {config.boundary_reference_grid}", flush=True)
    print(f"CUDA wait seconds: {int(args.cuda_wait_seconds)}", flush=True)
    print(f"CUDA retry sleep: {int(args.cuda_retry_sleep)}", flush=True)
...
def print_final_summary(quad_id: int, stats, ok: bool) -> None:
    ...
    print(f"Boundary param cache: {bool(stats['use_boundary_param_cache'])}", flush=True)
    print(f"Boundary vertices created by param cache: {int(stats['boundary_created_total'])}", flush=True)
    print(f"Boundary created a-b: {int(stats['boundary_created_a_to_b'])}", flush=True)
    print(f"Boundary created b-d: {int(stats['boundary_created_b_to_d'])}", flush=True)
    print(f"Boundary created c-d: {int(stats['boundary_created_c_to_d'])}", flush=True)
    print(f"Boundary created a-c: {int(stats['boundary_created_a_to_c'])}", flush=True)
    ...
    print(f"Timing vertex seconds: {float(stats['time_vertex_s']):.3f}", flush=True)
    print(f"Timing grid seconds: {float(stats['time_grid_s']):.3f}", flush=True)
    print(f"Timing grid model seconds: {float(stats['time_grid_model_s']):.3f}", flush=True)
    print(f"Timing grid CPU seconds: {float(stats['time_grid_cpu_s']):.3f}", flush=True)
    print(f"Timing grid stack seconds: {float(stats['time_grid_stack_s']):.3f}", flush=True)
    ...
    print(f"Timing repair GPU-sum seconds: {float(stats['time_repair_gpu_sum_s']):.3f}", flush=True)
    if ok:
        print(f"Final result: SUCCESS. Found a shared-vertex level-wise label-preserving filling of the loop a-b-d-c-a for label={int(stats['y'])} using boundary_param_cache={int(stats['use_boundary_param_cache'])}.", flush=True)
    else:
...
def main() -> None:
    ...
    with contextlib.redirect_stdout(tee), contextlib.redirect_stderr(tee):
        ...
        try:
            ...
            image_paths, tensors = load_quad_tensors(paths.quad_image_dir, quad_id, int(args.filename_width), preprocess)
            print("Quad image paths:", flush=True)
            for corner in ["a", "b", "c", "d"]:
                print(f"  {corner}: {image_paths[corner]}", flush=True)
            print()
            accepted, vertices, stats = fill_quad_surface(
                model,
                tensors["a"],
                tensors["b"],
                tensors["c"],
                ...
                config=config,
                paths=paths,
                checkpoint_prefix=f"quad_{quad_id:04d}",
                target_label=None,
            )
